...
function initLocalNode() {
  ...
  $('btn-ln-save').addEventListener('click', async () => {
    ...
    try {
      await apiJson('/api/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patch) });
      toast('配置已保存并生效');
      // 同步刷新：状态卡、配置表单、驾驶舱、事件日志（保证状态与配置一致显示）
      await loadConfig();
...
const CONFIG_FIELDS = [
  ...
  { group: '抓取', key: 'fetcher.max_per_source', label: '单源入库节点上限', type: 'number', hint: '单个来源一次抓取最多入库的节点数，0=不限制，防止海量订阅打爆节点池。' },
  ...
  { group: '抓取', key: 'grab.auto_cron', label: '自动采集 cron 表达式', type: 'text', hint: '优先级高于间隔，如 0 0 * * * = 每天 00:00；留空则按下方间隔（分钟）。' },
  { group: '抓取', key: 'grab.auto_interval_minutes', label: '自动采集间隔（分钟）', type: 'number', hint: '0=关闭；开启后定时抓取全部启用且参与采集的来源。' },
  { group: '抓取', key: 'grab.auto_probe', label: '抓取后自动测速', type: 'bool', hint: '抓取（手动/自动/定时采集）完成后自动对本次入库节点测速。' },
  { group: '抓取', key: 'grab.auto_remove_unreachable', label: '测速后删除不可用节点', type: 'bool', hint: '开启自动测速后生效：测速完成自动删除"已测且不可用"的节点，实现抓取→测速→过滤闭环。' },
