...
- **数据持久化与迁移**：配置/模板/节点池经可插拔存储层（默认文件存储），增量持久化；导出/导入 JSON 一键迁移
...
打开 http://localhost:8080 进入驾驶舱。在「全站参数」配置主订阅地址、令牌等；在驾驶舱输入订阅链接/网页/文本点击「抓取并入库」。
